    ...
    def run(self, env, parent, child):
        while True:
            if (len(self.packet_list) > 0) and (child is not None):
                ...
                self.transmit_slot = int_from_range(TX_RANGE)
                self.transmitting = True
                yield env.timeout(self.transmit_slot)
                self.transmitting = False
                ...
                receive_slot = int_from_range(RX_RANGE)
                ...
                if (parent is not None) and transmission_ticks_recorded == parent.transmit_slot and transmission_ticks_recorded != 0:
