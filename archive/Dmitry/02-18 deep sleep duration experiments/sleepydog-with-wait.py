    ...
    def run(self, env, parent, child):
        while True:
            if (len(self.packet_list) > 0) and (child is not None):
                ...
                sleepydog_time = convert_time_to_sleepydog_time(sleep_time)
                diff = sleep_time - sleepydog_time

                # If sleepydog underestimated, do sleep + extra wait
                # if overestimated(over 15), do extra wait only
                if diff > 0:
                    yield env.timeout(sleepydog_time)
                    tx_start_timestamp = env.now
                    yield env.timeout(diff)
                else:
                    tx_start_timestamp = env.now
