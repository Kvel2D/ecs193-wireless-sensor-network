...
def simulate():
    ...
    file.write("%f," % (rx_time_percent + tx_time_percent))
    ...
    for packet in nodes[NODE_COUNT - 1].packet_list:
        ...
        node_to_node_time = create_to_gateway_time / (NODE_COUNT - 1)
        if node_to_node_time > max_time:
            max_time = node_to_node_time
        if node_to_node_time < min_time:
            min_time = node_to_node_time
    ...
    file.write('%s,' % format_time(avg_packet_node_to_node_time))
    file.write('%s,' % format_time(min_time))
