...
void loop_rx() {
    ...
    bool before = rf69_manager.available();
    ...
    bool after = rf69_manager.available();
    
    if (before) {
        Serial.println("available before");
        uint8_t len = sizeof(buffer);
        rf69.recv(buffer, &len);
        bool after_after = rf69_manager.available();
        if (after_after) {
            Serial.println("available after_after");
        }
    }

    if (after) {
        Serial.println("available after");
    }
    ...
    if (received) {
        ...
        static uint16_t prev_packet_number = 100;
        if (prev_packet_number == p.number) {
            duplicate_count++;
        }
        total_count++;
        prev_packet_number = p.number;
        ...
        Serial.print("=");
        Serial.print(1.0f * duplicate_count / total_count * 100);
        Serial.println("%");
    }
}
    ...
    if (has_sensor[my_id]) {
        if (millis() - sensor_wakeup_time >= PACKET_PERIOD) {
            if (packet_queue.size < QUEUE_SIZE_MAX) {
                ...
                // REMOVE:
                new_packet.reading[0] = 24.0;
                new_packet.reading[1] = 25.0;
                new_packet.reading[2] = -127.0;
                new_packet.reading[3] = -127.0;
                new_packet.reading[4] = -127.0;
                new_packet.reading[5] = -127.0;
