...
#define PACKET_PERIOD (1000ul * 10ul)
...
void print_packet(struct Packet p) {
  // NOTE: old printing with multiple prints, remove when sure that it's not
  // needed Serial.print(p.age); Serial.print(","); Serial.print(p.current_id);
  // Serial.print(",");
  ...
  int print_size =
      snprintf(print_packet_buffer, sizeof(print_packet_buffer),
               "%lu,%u,%u,%u,%d.%01d,%d.%01d,%d.%01d,%d.%01d,%d.%01d,%d.%01d",
               p.age, p.current_id, p.number, p.origin_id, (int)(p.reading[0]),
               (int)(p.reading[0] * 100) % 100 / 10, (int)(p.reading[1]),
               (int)(p.reading[1] * 100) % 100 / 10, (int)(p.reading[2]),
               (int)(p.reading[2] * 100) % 100 / 10, (int)(p.reading[3]),
               (int)(p.reading[3] * 100) % 100 / 10, (int)(p.reading[4]),
               (int)(p.reading[4] * 100) % 100 / 10, (int)(p.reading[5]),
               (int)(p.reading[5] * 100) % 100 / 10);
...
void updatePacketAge(uint32_t sleep_time) {
...
void loop_tx() {
  ...
  do {
    ...
    if (tx_success) {
      ...
      tx_success &&
      packet_queue.size >
          0);  // if last transmission was successful and more packets exist
}
...
void loop() {
  ...
  if (my_data.has_sensor &&
      (do_first_reading_packet ||
       millis() - last_reading_time >= PACKET_PERIOD) &&
      packet_queue.size < QUEUE_SIZE_MAX) {
  ...
  // Update packet ages
  uint32_t iteration_time = millis() - temp_timer;
  updatePacketAge(iteration_time);
}
